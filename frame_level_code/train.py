...
if __name__ == "__main__":
  ...
  flags.DEFINE_bool(
      "use_cv", False,
      "Set true if you want to train on a fold"
      "logs on startup.")
  flags.DEFINE_integer("fold", 0,
                       "Choose which fold you will use.")
  flags.DEFINE_integer("split_seed", 0,
                       "Use this seed for K-fold CV split.")
...
def get_input_data_tensors(reader,
  ...
  with tf.name_scope("train_input"):
    if FLAGS.use_cv:
        files = split_fold(data_pattern, fold=FLAGS.fold, split_seed=FLAGS.split_seed)
    else:
        files = gfile.Glob(data_pattern) + [fname.replace('train', 'validate') for fname in gfile.Glob(data_pattern)]
    if not files:
