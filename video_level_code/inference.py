...
def inference(reader, train_dir, data_pattern, out_file_location, batch_size, top_k):
 ...
 config = tf.ConfigProto(allow_soft_placement=True,  device_count = {'GPU': FLAGS.gpu_only})
 ...
 with tf.device(device):
  with tf.Session(config=config) as sess, gfile.Open(out_file_location, "w+") as out_file:
    ...
    if FLAGS.use_ema_var:
        ...
        ckpt_reader = pywrap_tensorflow.NewCheckpointReader(latest_checkpoint)
        #var_to_shape_map = ckpt_reader.get_variable_to_shape_map()
        for v in slim.get_model_variables():
          ema_var_name = 'tower/'+v.op.name+'/ExponentialMovingAverage'
          sess.run(tf.assign(v, ckpt_reader.get_tensor(ema_var_name)))
          logging.info("Replaced {} with {}.".format(v.op.name, ema_var_name))
