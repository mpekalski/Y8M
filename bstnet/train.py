...
if __name__ == "__main__":
  ...
  flags.DEFINE_string(
      ...
      "in models.py.")
  flags.DEFINE_string("bst_model", "NN2Lc",
      "Use a boost model.")
  flags.DEFINE_bool(
  ...
  flags.DEFINE_string("c_vars","","A list of computed variables. Available transformations sq_a log_a inv_a abs_a sqrt_a diff_a_b interaction__a_b") 
  ...
  flags.DEFINE_float("bst_learning_rate", 0.01,
                     "learning rate for bst_model.")
...
def get_input_data_tensors(reader,
  ...
  with tf.name_scope("train_input"):
    files1 = gfile.Glob(data_pattern)
    ...
    if not files:
      raise IOError("Unable to find training files. data_pattern='" +
                    data_pattern + "'.")
    logging.info("Number of training files: %s + %s + %s =  %s.", 
...
def build_graph(reader,
  ...
  wgts_np = numpy.ones(FLAGS.truncated_num_classes)
  ...
  bst_optimizer = optimizer_class(bst_learning_rate)
  ...
  if FLAGS.apply_global_normalization:
     model_input = tf.multiply(tf.subtract(model_input_raw, global_mean_padded), global_inv_std_padded)
        ...
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus!=1 else "/gpu:0")): 
          ...
          if "loss" in result.keys():
            ...
            label_loss = label_loss_fn.calculate_loss(
                    ...
                    FLAGS.loss_epsilon, wgts=wgts_4_lossfn)
          ...
          pred_diff = tf.cast(tower_labels[i], tf.float32) - predictions
          pred_diff_as_target = tf.stop_gradient(pred_diff)

          bst_results = bst_model.create_model(
                  tower_inputs[i],
                  num_frames=tower_num_frames[i],
                  vocab_size=reader.num_classes,
                  layers_keep_probs=layers_keep_probs,
          ...
          bst_loss = losses.L2Loss().calculate_loss(
                  bst_results["predictions"], pred_diff_as_target)
          
          bst_predictions = predictions + bst_results["predictions"]
          tower_bst_predictions.append(bst_predictions)
          tower_bst_losses.append(bst_loss)
          bst_gradients = bst_optimizer.compute_gradients(
                  bst_loss, colocate_gradients_with_ops=False)
          tower_bst_gradients.append(bst_gradients)
          ...
          e_result = model.create_model(
            ...
            layers_keep_probs=tf.ones([10], tf.float32, name="keep_probs_1"),
            is_training=False
           )
          
          e2_result= bst_model.create_model(
                  e_tower_inputs[i],
                  num_frames=e_tower_num_frames[i],
                  vocab_size=reader.num_classes,
                  is_training=False)
          ...
          e2_predictions = e_predictions + e2_result["predictions"]
          e2_tower_predictions.append(e2_predictions)
          # end eval ops
  ...
  # second level loss
  bst_label_loss = tf.reduce_mean(tf.stack(tower_bst_losses))
  tf.summary.scalar("bst_label_loss", bst_label_loss)
  merged_bst_gradients = utils.combine_gradients(tower_bst_gradients)
  bst_train_op = bst_optimizer.apply_gradients(merged_bst_gradients, global_step=global_step)
  ...
  tf.add_to_collection("bst_predictions", tf.concat(tower_bst_predictions,0))
  tf.add_to_collection("bst_label_loss", bst_label_loss)
  tf.add_to_collection("bst_train_op", bst_train_op)
  ...
  e2_label_loss = losses.L2Loss().calculate_loss(
          tf.concat(e2_tower_predictions, 0), e_labels_batch)
  ...
  tf.add_to_collection("e2_predictions", tf.concat(e2_tower_predictions, 0))
  tf.add_to_collection("e2_loss", e2_label_loss)
  
class Trainer(object):
  """A Trainer to train a Tensorflow graph."""

  def __init__(self, cluster, task, train_dir, model, bst_model, 
               reader, model_exporter,
...
#                           task_as_string(self.task))
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device(device_fn):
        if not meta_filename:
          saver = self.build_model(self.model, self.bst_model, self.reader)
        ...
        # boost phase
        bst_loss = tf.get_collection("bst_label_loss")[0]
        bst_predictions = tf.get_collection("bst_predictions")[0]
        bst_train_op = tf.get_collection("bst_train_op")[0]
        
        # eval
        e_loss = tf.get_collection("e_loss")[0]
        e_labels = tf.get_collection("e_labels")[0]
        e_predictions = tf.get_collection("e_predictions")[0]
        e2_loss = tf.get_collection("e2_loss")[0]
        e2_predictions = tf.get_collection("e2_predictions")[0]
    ...
    with sv.managed_session(target, config=self.config) as sess:
      try:
        ...
        while (not sv.should_stop()) and (not self.max_steps_reached):
          ...
          _, _, global_step_val, loss_val, predictions_val, labels_val, \
          bst_predictions_val, bst_loss_val = sess.run(
              [train_op, bst_train_op, global_step, loss, predictions, labels,
               bst_predictions, bst_loss])
          ...
          if self.is_master and global_step_val % 20 == 0 and self.train_dir:
            hit_at_one = eval_util.calculate_hit_at_one(predictions_val, labels_val)
            perr = eval_util.calculate_precision_at_equal_recall_rate(predictions_val,
                                                                      labels_val)
            gap = eval_util.calculate_gap(predictions_val, labels_val)
            bst_gap = eval_util.calculate_gap(bst_predictions_val, labels_val)
            ...
            sv.summary_writer.add_summary(
                utils.MakeSummary("model/bst_GAP", bst_gap), global_step_val)
            sv.summary_writer.add_summary(
                utils.MakeSummary("global_step/Examples/Second",
                                  examples_per_second), global_step_val)
            
            #also do eval
            e_loss_val, e_predictions_val, e_labels_val, e2_loss_val, e2_predictions_val = sess.run(
              [e_loss, e_predictions, e_labels, e2_loss, e2_predictions])
            ...
            e2_gap = eval_util.calculate_gap(e2_predictions_val, e_labels_val)
            ...
            sv.summary_writer.add_summary(
                utils.MakeSummary("model/E2_GAP", e2_gap), global_step_val)
  ...
  def build_model(self, model, bst_model, reader):
    ...
    build_graph(reader=reader,
                 ...
                 bst_learning_rate=FLAGS.bst_learning_rate,
...
def main(unused_argv):
  ...
  with open(os.path.join(FLAGS.train_dir, 'flags.cfg'),'w') as ff:
  ...
  if not cluster or task.type == "master" or task.type == "worker":
    ...
    bst_model = find_class_by_name(FLAGS.bst_model,
        [frame_level_models, video_level_models,
         xp_frame_level_models, xp_video_level_models])()
        
    reader = get_reader()

    # as i understand, exporter only needs layer_keep_probs for predictions, so those should always be 1.0
    ...
    Trainer(cluster, task, FLAGS.train_dir, model, bst_model, reader, model_exporter,
